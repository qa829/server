...
{
    ## Check if the server has the TIME_MS column on the
    ## INFORMATION_SCHEMA.PROCESSLIST table (MariaDB and Percona Server),
    ...
    if ($has_time_ms == 1)
    {
        ## Check if the server has the STAGE column
        # on the INFORMATION_SCHEMA.PROCESSLIST table (MariaDB) to retreive
        # query completion informations
...
#         print("q_diff: $STATUS{Questions} - $OLD_STATUS{Questions}  / $t_delta = $q_diff\n");
    ...
    foreach my $thread (@data)
    {
        ...
        ## If Progress information is available and a multi-stage query is running, 
        ## the actual stage and the total number of stages of the thread are shown 
        ## at the beginning of the State column
